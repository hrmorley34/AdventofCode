records = """[1518-11-07 00:21] falls asleep
...
def time2mins(time_s):
    time_i = time_s.split(":")
    time_i = list(map(int, time_i))
    return(time_i[0]*60 + time_i[1])
def next_date(d):
    y, m, d = map(int, d.split("-"))
    d += 1
    if (d>31 and m in (1,3,5,7,8,10,12)) or (d>30 and m in (4,6,9,11)) or (d>28 and m==2):
        m += 1
        d = 1
    if m > 12:
        m = 1
        y += 1
    return("-".join(map(lambda i: str(i).rjust(2,"0"), (y, m, d))))

guards = {}
c_guard = None
for record in records:
    c_date, c_time = record[1:17].split(" ")
    if time2mins(c_time) >= 720:
        c_date = next_date(c_date)
        c_time_i = time2mins(c_time) - 1440
    else:
        c_time_i = time2mins(c_time)
    c_date_i = c_date.split("-")
    c_date_i = list(map(int, c_date_i))
    data = record[19:]
    if data.startswith("Guard #"):
        c_guard = int(data.split(" ")[1][1:])
        guards[c_date] = [c_guard, {c_time_i:1}]
    elif data.startswith("falls"):
        for x in range(max(guards[c_date][1].keys())+1, time2mins(c_time)):
            guards[c_date][1][x] = 1
        guards[c_date][1][c_time_i] = 0
    elif data.startswith("wakes"):
        for x in range(max(guards[c_date][1].keys())+1, time2mins(c_time)):
            guards[c_date][1][x] = 0
        guards[c_date][1][c_time_i] = 1

guardtimes = {}
for date, (guard, times) in guards.items():
    guardtimes[guard] = guardtimes.get(guard, {})
    guardtimes[guard][date] = sum(map(lambda x: 1-x, times.values()))
guardtotals = {}
sleepyguard = None
for guard, datetimes in guardtimes.items():
    guardtotals[guard] = sum(datetimes.values())
    if guardtotals[guard] == max(guardtotals.values()):
        sleepyguard = guard

timetotals = {}
for time in range(0, 60):
    timetotals[time] = 0
    for date in guardtimes[sleepyguard].keys():
        timetotals[time] += 1 - guards[date][1].get(time, 1)
top_length = (None,0)
for minute, length in timetotals.items():
    if length >= top_length[1]:
        top_length = (minute, length)

s1 = top_length[0] * sleepyguard
print("S1:", s1)

timetotals = {}
for date, (guard, times) in guards.items():
    timetotals[guard] = timetotals.get(guard, {})
    for time, wake in times.items():
        timetotals[guard][time] = timetotals[guard].get(time, 0) + 1 - wake
top_minute = (None, 0, 0)
for guard, mt in timetotals.items():
    for minute, times in mt.items():
        if times >= top_minute[2]:
            top_minute = (guard, minute, times)

s2 = top_minute[0] * top_minute[1]
print("S2:", s2)
