...
poses = """position=<-20620, -41485> velocity=< 2,  4>
...
def split_pos_vel(s):
    x = -1
    x = pos_start = s.find("<", x+1)
    x = pos_end = s.find(">", x+1)
    x = vel_start = s.find("<", x+1)
    x = vel_end = s.find(">", x+1)
    s_pos, s_vel = s[pos_start+1:pos_end], s[vel_start+1:vel_end]
    i_pos = list(map(lambda x: int(x.strip(" ")), s_pos.split(", ")))
    i_vel = list(map(lambda x: int(x.strip(" ")), s_vel.split(", ")))
    return([i_pos, i_vel])
poses = poses.split("\n")
poses = list(map(split_pos_vel, poses))

def go(poses, n=1):
    o_poses = []
    for p in poses:
        o_poses.append([[p[0][0]+(p[1][0]*n), p[0][1]+(p[1][1]*n)], p[1]])
    return(o_poses)

pygame.init()
SIZE = (1000, 600)
PIX_SIZE = (1, 1)
DISPLAY_OFFSET = (SIZE[0]//2, SIZE[1]//2)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RIGHT:
                ...
                n = -1
            if event.mod & (pygame.KMOD_CTRL | pygame.KMOD_LCTRL | pygame.KMOD_RCTRL):
                n *= 25
            if event.mod & (pygame.KMOD_SHIFT | pygame.KMOD_LSHIFT | pygame.KMOD_RSHIFT):
                n *= 5
            poses = go(poses, n)
            c_state += n

    screen.fill((0,0,0))
    for point, vel in poses:
        if point[0] in range(0, SIZE[0]//PIX_SIZE[0]) and point[1] in range(0, SIZE[1]//PIX_SIZE[1]):
            new_point = [point[0] * PIX_SIZE[0] + DISPLAY_OFFSET[0],
                         point[1] * PIX_SIZE[1] + DISPLAY_OFFSET[1]]
            pygame.draw.rect(screen, (255,255,255), pygame.Rect(new_point, PIX_SIZE))
    pygame.display.flip()
